...
const apiKey='AIzaSyB1KbTLxyWYHQ9MS3ox9TXbc-LsFUwucnc'

function query() {

  books=loadFromStorage('Books') || books
...
function addReview(bookId, review) {
  ...
  !selectedBook.reviews ? selectedBook.reviews = [reviewToAdd] : selectedBook.reviews.push(reviewToAdd)
  saveToStorage('Books', books)
  return Promise.resolve(selectedBook.reviews)
}

function removeReview(reviewId, bookId) {
  const selectedBook = books.find(book => book.id === bookId)
  const selectedReview = selectedBook.reviews.findIndex(review => review.id === reviewId)
  selectedBook.reviews.splice(selectedReview, 1)
  return Promise.resolve(selectedBook.reviews)
}
...
today = yyyy + '-' + mm + '-' + dd;
  return { name: 'Books Reader', date: today };
}
...
function searchBooks(input){
  const searchedBook=fetch(`https://www.googleapis.com/books/v1/volumes?q=${input}:keyes&key=${apiKey}`)
  .then(res=> res.json()
...
function getNextPrev(bookId){

  const currBookIdx=books.findIndex(book=> book.id===bookId)

  const prevBook= books[currBookIdx-1] || books[books.length-1]
  const nextBook = books[currBookIdx+1] || books[0]

  return {
    prevBook: prevBook.id,
    nextBook: nextBook.id
  }
